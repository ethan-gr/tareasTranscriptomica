...
colnames(fc$counts) <- c("m5xFAD-A", "WT-A", "m5xFAD-B", "WT-B")
...
barplot(PCA_perc_var, names=colnames(PCA_log2CPM$x), 
        main="Scree plot RNAseq hippocampus", xlab="Principal Components", ylab="Percent Variation")
...
plot(PCA_log2CPM$x[,"PC1"], PCA_log2CPM$x[,"PC2"], 
     main="PCA of RNAseq from hippocampus", xlab=PCA_var_lab[1], ylab=PCA_var_lab[2], col=colors, pch=19)
...
contrasts <- makeContrasts(WTvsm5xFAD=m5xFAD-WT, levels=design)
lrt_hip <- glmLRT(fit, contrast=contrasts[, "WTvsm5xFAD"])
...
plot(DEG_lrt$logFC[!(up | down)], -log10(DEG_lrt$FDR[!(up | down)]), pch=19, col="gray", cex=0.4, 
     xlab="log2 Expression fold change", ylab="-log10 FDR", main="Volcano plot WTvs5xFAD", xlim=c(-10,10),ylim=c(0,30))
